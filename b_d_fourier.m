...
%     temp_OD=node_extract(nodes_FEM_OD_WT,displacement_FEM_OD_WT);
    ...
    temp_CD=[temp_CD displacement_FEM_CD_WT(:,2:4)];
...
%     [theta_FEM_OD_WT,r_FEM_OD_WT]=cart2pol(temp_OD(:,indx),temp_OD(:,indy));
    temp_CD(:,2:3)=temp_CD(:,2:3)-C_FEM_CD_WT(:,1:2);
    [theta_FEM_CD_WT,r_FEM_CD_WT]=cart2pol(temp_CD(:,indx),temp_CD(:,indy));
    
    dr_FEM_CD_WT=sqrt(temp_CD(:,indx+3).^2+temp_CD(:,indy+3).^2);
    
    temp_CD=[temp_CD theta_FEM_CD_WT r_FEM_CD_WT dr_FEM_CD_WT];
...
x=zeros(length(r_FEM_CD_WT),1);
y=zeros(length(r_FEM_CD_WT),1);
u=zeros(length(r_FEM_CD_WT),1);
phi=zeros(length(r_FEM_CD_WT),1);
