...
%     displacement_CMM_OD_WO=importdata([address_CMM_OD_WO displacement_names_CMM(1,:)]);
    ...
    Cx_CD=mean(temp_CD(:,2));
    Cx_OD=mean(temp_OD(:,2));
        
    temp_CD(:,2)=temp_CD(:,2)-Cx_CD;
    temp_OD(:,2)=temp_OD(:,2)-Cx_OD;
    
    deformation_CD=temp_CD(:,2:4)+displacement_FEM_CD_WT(:,2:4);
    deformation_OD=temp_OD(:,2:4)+displacement_FEM_OD_WT(:,2:4);
...
%     R_CMM_OD_WO=norm(displacement_CMM_OD_WO(1,1:3)-C_CMM_OD_WO);
    ...
    R_FEM_OD_WT=norm(deformation_OD(1,:)-C_FEM_OD_WT);
    R_FEM_CD_WT=norm(deformation_CD(1,:)-C_FEM_CD_WT);
...
%     [theta_CMM_OD_WO,r_CMM_OD_WO]=cart2pol(displacement_CMM_OD_WO(:,indx),displacement_CMM_OD_WO(:,indy));
    ...
    [theta_FEM_OD_WT,r_FEM_OD_WT]=cart2pol(deformation_OD(:,indx),deformation_OD(:,indy));
    [theta_FEM_CD_WT,r_FEM_CD_WT]=cart2pol(deformation_CD(:,indx),deformation_CD(:,indy));
...
%     dr_CMM_OD_WO=r_CMM_OD_WO-39.3;
    ...
    dr_FEM_CD_WT=r_FEM_CD_WT-39.3;
    dr_FEM_OD_WT=r_FEM_OD_WT-39.3;
...
%     temp_CMM_OD_WO=[displacement_CMM_OD_WO(:,1:3) theta_CMM_OD_WO r_CMM_OD_WO dr_CMM_OD_WO];
    temp_FEM_CD_WT=[deformation_CD(:,1:3) theta_FEM_CD_WT r_FEM_CD_WT dr_FEM_CD_WT];
    temp_FEM_OD_WT=[deformation_OD(:,1:3) theta_FEM_OD_WT r_FEM_OD_WT dr_FEM_OD_WT];
...
%     temp_CMM_OD_WOn=[temp_CMM_OD_WO;temp_CMM_OD_WO(1,:)];
    ...
    temp_FEM_CD_WT=sortrows(temp_FEM_CD_WT,4);
    temp_FEM_OD_WT=sortrows(temp_FEM_OD_WT,4);
...
%     CMM_OD_WOn{SecNo_CMM,LinerNo}=temp_CMM_OD_WOn;
    ...
    FEM_OD_WT{SecNo_FEM,LinerNo}=temp_FEM_OD_WT;
...
%     figure
    ...
    polar(CMM_CD_WT{SecNo_CMM,LinerNo}(:,4),CMM_CD_WT{SecNo_CMM,LinerNo}(:,5)+scale*CMM_CD_WT{SecNo_CMM,LinerNo}(:,6),'m');hold on
%     polar(CMM_CD_WOn{SecNo_CMM,LinerNo}(:,4),CMM_CD_WOn{SecNo_CMM,LinerNo}(:,5)+scale*CMM_CD_WOn{SecNo_CMM,LinerNo}(:,6),'c');hold on
    polar(CMM_OD_WT{SecNo_CMM,LinerNo}(:,4),CMM_OD_WT{SecNo_CMM,LinerNo}(:,5)+scale*CMM_OD_WT{SecNo_CMM,LinerNo}(:,6),'b');hold on
%     polar(CMM_OD_WO{SecNo_CMM,LinerNo}(:,4),CMM_OD_WO{SecNo_CMM,LinerNo}(:,5)+scale*CMM_OD_WO{SecNo_CMM,LinerNo}(:,6),'r')
    axis equal
    
%     figure
%     subplot(1,4,5-LinerNo)
    polar(FEM_CD_WT{SecNo_FEM,LinerNo}(:,4),39.3+scale*FEM_CD_WT{SecNo_FEM,LinerNo}(:,6),'c');hold on
    polar(FEM_OD_WT{SecNo_FEM,LinerNo}(:,4),39.3+scale*FEM_OD_WT{SecNo_FEM,LinerNo}(:,6),'r');hold on
    axis equal
%     
...
%     dr_CMM_OD_WT
%     dr_CMM_OD_WO
      DIS=[DIS;max(dr_FEM_CD_WT) max(dr_CMM_CD_WT) max(dr_FEM_OD_WT) max(dr_CMM_OD_WT)]
